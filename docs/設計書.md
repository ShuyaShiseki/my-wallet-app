...
- 認証: Firebase設定時は匿名認証を利用する。未設定時はローカル利用を継続する
...
4. Firebase が設定済みの場合、匿名認証を開始する。
5. 認証後、Firestore の `wallets/my-wallet-app` を読み込む。
6. Firestore にデータが存在する場合は、ローカル状態よりリモート状態を採用する。
7. 読み込み完了後、画面操作と状態の保存を有効にする。
...
- 操作ボタンは押下中に縮小・半透明化し、押下状態を視覚的に示す。
...
- 銀行から財布への引き出し額を入力する。
- 引き出し額が0以下の場合は登録しない。
- 銀行残高を超える引き出しは登録せず、iOS標準の警告ダイアログと画面内エラーメッセージを表示する。
- 登録時、銀行残高を減らし、財布残高を増やす履歴を作成する。
- 履歴上のカテゴリは「引き出し」とする。

### 3.2 履歴

- 支出と引き出しを作成日時の降順で表示する。
- 支出合計、引出合計、今月の支出を表示する。
- 各履歴に種別、対象口座、カテゴリ、日時、金額を表示する。
- 支出金額はマイナス、引き出し金額はプラスとして表示する。
- 削除操作時は確認ダイアログを表示する。
- 確認後に対象履歴を削除し、残高を再計算する。
...
type HistoryType = 'expense' | 'withdrawal'
...
| 支出・引き出しが残高を超過 | 登録を拒否し、iOS標準の警告ダイアログと不足を示す画面内エラーメッセージを表示 |
...
| Firebase環境変数不足 | Firebase同期・匿名認証を無効化し、AsyncStorageのみで継続 |
| Firebase匿名認証に失敗 | 警告ログ後、取得できたローカル状態で継続 |
| 履歴0件 | 空状態を表示 |
...
- Firebase設定時はFirebase Authenticationの匿名認証を利用する。
- Firestoreのアクセス制御はアプリコードではなくFirebase側のルールで設定する必要がある。
- 現行のFirestoreドキュメントは単一ドキュメントであり、複数ユーザー単位のデータ分離は未対応である。
...
- 同時編集時の競合解決は行わない。
- 匿名認証は導入済みだが、ユーザーID単位のFirestoreデータ分離は未対応である。
...
1. Firebase Authentication のメールアドレス・Appleログイン対応
